...
import { ConversationAwarenessVolume } from "./bgCA";
...
const Content: FC<{ backend: Backend }> = ({ backend }) => {
  ...
  const onConnectionChanged = (state: BackendSocketState) => {
    console.log("Index: connection state:", state);

    if (state === BackendSocketState.OPEN) {
      setConnectionErrorValue(false);
      setIsButtonDisabledValue(true);
      setUpdateRequiredValue(false);
      backend.getAll();
    }

    if (state === BackendSocketState.ERROR || state === BackendSocketState.CLOSED) {
      setConnectionErrorValue(true);
      setIsButtonDisabledValue(false);
      setUpdateRequiredValue(false);
    }

    if (state === BackendSocketState.INVALIDVERSION) {
      setConnectionErrorValue(false);
      setIsButtonDisabledValue(true);
      setUpdateRequiredValue(true);
    }
  ...
  const onJsonMessageReceived = (lastJsonMessage: object) => {
    ...
    if (typedJson?.settings != null){
        ...
        const mpLogLevel = typedJson?.settings?.["magicpods"]?.["logLevel"];
        const parsedLogLevel = Number(mpLogLevel);
        if (!isNaN(parsedLogLevel)) {
          setCoreLogLevel(parsedLogLevel);
        }
    }
  ...
  const [coreLogLevel, setCoreLogLevel] = useState<number>(0);
      ...
      <Tabs
        ...
        tabs={[
          ...
          },
          {
            ...
            content: <TabSettings backend={backend} toggleAnimationOn={toggleAnimationOn} setToggleAnimationOn={setToggleAnimationOn} coreLogLevel={coreLogLevel} setCoreLogLevel={setCoreLogLevel} />,
            id: "settings",
          }
